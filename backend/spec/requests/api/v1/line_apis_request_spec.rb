...
RSpec.describe "Api::V1::LineApisController", type: :request do
  describe '#webhock' do
    ...
    before do
      ...
      allow_any_instance_of(Api::V1::LineApisController).to receive(:reply)
      allow_any_instance_of(Api::V1::LineApisController).to receive(:validate_signature)
      allow_any_instance_of(Api::V1::LineApisController).to receive(:events) { double_events }
    end
    ...
    describe 'レスポンス' do
      before do
        post webhock_api_v1_line_api_path
      end
    ...
    describe '初期の位置情報設定' do
      context 'ユーザーが初めてアクセスした時' do
        it 'Lineユーザーを作成すること' do
          expect { post webhock_api_v1_line_api_path }.to change(LineUser, :count).by(1)
        end
      end
      ...
      context 'ユーザーのアクセスが2回目の時' do
        ...
        it 'Lineユーザーを作成しないこと' do
          post webhock_api_v1_line_api_path
          expect { post webhock_api_v1_line_api_path }.to_not change(LineUser, :count)
        end
      end
      ...
      context '無効なテキストイベントを受け取った時' do
        ...
        it 'Weatherを作成しないこと' do
          expect { post webhock_api_v1_line_api_path }.to_not change(Weather, :count)
        end
      end

      context '有効なテキストイベントを受け取った時' do
        it 'Weatherを作成すること' do
          expect { post webhock_api_v1_line_api_path }.to change(Weather, :count).by(1)
        end
      end
